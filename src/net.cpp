...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "TrustPlus " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
// retired {"MitchellMint.com", "184.173.115.98"},
	{"trust.digital", "104.197.97.72"},
	{"trustplus.co", "23.251.149.70"}
...
void ThreadDNSAddressSeed(void* parg)
{
    ...
    RenameThread("TrustPlus-dnsseed");
...
void ThreadDumpAddress(void* parg)
{
    ...
    RenameThread("TrustPlus-adrdump");
...
void ThreadOpenConnections(void* parg)
{
    ...
    RenameThread("TrustPlus-opencon");
...
void ThreadMessageHandler(void* parg)
{
    ...
    RenameThread("TrustPlus-msghand");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. TrustPlus is probably already running."), addrBind.ToString().c_str());
        else
